...
func (c *ctx) convertMode(n cc.ExpressionNode, w writer, s *buf, from, to cc.Type, fromMode, toMode mode) (r *buf) {
	...
	case exprUintpr:
		...
		case exprCall:
			...
			w.w("*(*%suintptr)(%s) = %s;", tag(preserve), unsafeAddr(v), s) // Free pass from .pin
			var b buf
			b.w("%s", v)
			return &b
		}
...
func (c *ctx) logicalAndExpression(w writer, n *cc.LogicalAndExpression, t cc.Type, mode mode) (r *buf, rt cc.Type, rmode mode) {
	...
	case cc.LogicalAndExpressionLAnd: // LogicalAndExpression "&&" InclusiveOrExpression
		...
		case al.len() != 0 || ar.len() != 0:
			...
			w.w("%s; %s := %s; %s;", al.bytes(), v, bl, ar.bytes())
			b.w("((%s) && (%s))", v, br)
		default:
			b.w("((%s) && (%s))", bl, br)
		}
		...
		case *cc.PointerType:
			...
			case exprUintpr:
				...
				if x.Elem().Kind() == cc.Array {
					...
					case d != nil && d.Type().Kind() == cc.Ptr:
						mode2 = exprDefault
					}
				}
				b.w("%suintptr(%[1]sunsafe.%[1]sAdd(%[1]sunsafe.%[1]sPointer(%s), (%s%s)))", tag(preserve), c.expr(w, n.PostfixExpression, nil, mode2), c.expr(w, n.ExpressionList, nil, exprDefault), s)
			default:
